/** Server-side mirror of src/lib/knowledgeBase.ts — keep in sync with product deck. */
...
export const KNOWLEDGE_SECTIONS = [
  {
    id: 'problem',
    title: '1. Problem',
    keywords: ['problem', 'pollution', 'traffic', 'highway', 'india', 'old vehicles', 'health', 'environment', 'dpf', 'scr', 'existing', 'retrofit', 'portable'],
...
Existing solutions are aftermath-oriented (treatment after exposure), conventional static Diesel Particulate Filters (ceramic honeycomb soot capture), and Selective Catalytic Reduction (SCR) fluid+catalyst NOx systems — expensive and slow to replace. AeroPure does not modify engines; it is a portable external filter fitted outside the engine as a retrofit.`,
  },
  {
    id: 'customers',
    title: '2. Customer Segments',
    keywords: ['customer', 'fleet', 'logistics', 'vehicle owner', 'msme', 'genset', 'generator', 'industrial', 'diesel'],
...
Segment 2 — MSME Industries: small manufacturing, commercial complexes, workshops with diesel backup generators or localized industrial exhaust need affordable PCB compliance. AeroPure is compact, low-backpressure, multi-stage — efficient particulate scrubbing and thermal management without wet-scrubber footprint or water use.`,
  },
  {
    ...
    keywords: ['usp', 'value', 'benefit', 'backpressure', 'cots', 'modular', 'docking', 'pcb', 'puc', 'monitoring', 'tagline'],
...
Tagline: Modular Filtration, Zero Compromise: Smart Exhaust Purification for Vehicles and MSME.`,
  },
  {
    ...
    keywords: ['competitor', 'competitive', 'advantage', 'size', 'cost', 'lifetime', 'running cost', 'compare'],
...
• Lifetime: longer (~10 years) vs not applicable for many legacy systems`,
  },
  {
    id: 'solution',
    title: '5. Solution',
    keywords: ['solution', 'overview', 'sensor', 'sdg', 'trl', 'fit', 'portable', 'membrane', 'nano'],
...
SDG: 3 (Good Health), 9 (Industry/Innovation), 11 (Sustainable Cities). TRL: 3.`,
  },
  {
    ...
    keywords: ['charcoal', 'carbon', 'zeolite', 'nano membrane', 'pore', 'voc', 'macro', 'molecular sieving', 'layer 1', 'layer 2', 'layer 3', 'nanofiltration'],
...
• Layer 3 (Nano Membrane): small fine pores — high-density barrier against microscopic particulates and complex chemical structures.`,
  },
  {
    ...
    keywords: ['synthetic', 'pm10', 'pm2.5', 'electrostatic', 'nanofiber', 'ptfe', 'microfiber', 'dust', 'odor', 'array', 'graded'],
...
• Layer 6 (PTFE Protective Membrane): durable chemical-resistant PTFE barrier protecting filter array integrity before clean air exits.`,
  },
  {
    ...
    keywords: ['heat sink', 'oil', 'lubricant', 'fan', 'alloy', 'cooling', 'docking', 'quick-disconnect', 'thermal', 'backpressure', 'delta'],
...
• Self-Sealing Docking Mechanism at inlet; Quick-Disconnect Coupling at outlet — ENGINEERED FOR MODULAR SERVICE.`,
  },
  {
    id: 'channels',
    title: '6. Channels',
    keywords: ['channel', 'distribution', 'garage', 'aftermarket', 'workshop', 'b2b', 'sales', 'dealer'],
  ...
  },
  {
    ...
    keywords: ['revenue', 'hardware', 'cartridge', 'subscription', 'iot', 'dashboard', 'recurring', 'money', 'pricing'],
  ...
  },
  {
    ...
    keywords: ['cost', 'cogs', 'rupee', '₹', 'budget', 'production', 'casing', 'price'],
...
Total estimated COGS: ₹5,700 – ₹8,700 per unit (low-cost COTS production).`,
  ...
  },
  {
    ...
    keywords: ['moat', 'unfair', 'advantage', 'switching', 'scalability', 'proprietary', 'ecosystem'],
...
export function retrieveSections(agentId, message, limit = 4) {
  ...
  if (top.length === 0) return pool.slice(0, Math.min(3, pool.length));
  return top;
}

function summarizeSim(snap) {
  if (!snap || typeof snap !== 'object') return '';
  const parts = [];
  const push = (k, label, unit = '') => {
    const v = snap[k];
    if (typeof v === 'number' && Number.isFinite(v)) parts.push(`${label} ${v.toFixed(1)}${unit}`);
  };
  push('inletTempC', 'inlet', '°C');
  push('outletTempC', 'outlet', '°C');
  push('deltaT', 'ΔT', '°C');
  push('filtrationEfficiencyPct', 'eff', '%');
  push('pressureDropPa', 'ΔP', ' Pa');
  push('fanRpm', 'fan', ' rpm');
  push('filterLoadingPct', 'load', '%');
  return parts.length ? parts.join(' · ') : '';
}

export function answerFromKnowledge(agentId, message, simSnapshot) {
  ...
  const simLine = summarizeSim(simSnapshot);
  ...
  const markdown = [
    ...
    '',
    `Regarding: *${(message || 'overview').trim()}*`,
    '',
    body,
    simLine ? `\n\n**Live sim context (educational):** ${simLine}` : '',
    '',
    '---',
